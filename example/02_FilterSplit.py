...
def main():
    randint_stream = api.IStream(RandInt, 0, 100, sleep_sec=1e-7, max_records=NUM_RECORDS, fixed_to=['cloko000'])
    lo_stream, hi_stream = api.Operator(randint_stream, FilterSplit, 'num < 50', 'num >= 50')
    api.OStream(lo_stream, LocalFile, LOW_OUTPUT_FILE,  output_format='json', fixed_to=['cloko000'])
    api.OStream(hi_stream, LocalFile, HIGH_OUTPUT_FILE, output_format='json', fixed_to=['cloko000'])
